...
if (isset($_GET['del']))
{
	...
	{
		$result = $mysqli->query('SELECT image FROM article WHERE imageOrder=' . $_GET['del']);
		$row = $result->fetch_assoc();
		$result->free();
		if ($row)
		{
			unlink($_SERVER['DOCUMENT_ROOT'] . '/images/articles/' . $row['image']);
			$mysqli->query('DELETE FROM article WHERE imageOrder=' . $_GET['del']);
		}
	}
}
?>
...
<thead><tr><th>מיקום</th><th>כותרת</th><th>תמונה</th><th>עריכה</th><th>מחיקה</th></tr></thead>
<tbody>
<?php
$result = $mysqli->query('SELECT imageOrder, imageName, image FROM article ORDER BY imageOrder');
while ($row = $result->fetch_assoc())
{
	if (strpos($row['image'], '.svg') !== false)
		$width = $height = 10000;
	else
		list($width, $height) = getimagesize($_SERVER['DOCUMENT_ROOT'] . '/images/articles/' . $row['image']);
	echo '<tr>
...
<td data-label="תמונה"><a class="imageModal" href="/images/articles/' . $row['image'] . '" data-width="' . $width . '" data-height="' . $height . '"><img src="/images/articles/' . $row['image'] . '"></a></td>
<td data-label="עריכה"><a title="עריכה" href="/private/updatearticle.php?id=' . $row['imageOrder'] . '">עריכה</a></td>
...
function del(num)
{
	...
	(
		{
			title: 'האם אתה בטוח?',
			text: 'האם אתה בטוח שברצונך למחוק מאמר זה?',
			type: 'warning',
		...
		function()
		{
			self.location.href = '/private/updatearticles.php?del=' + num;
		}
	);
}
function edit(node)
...
function save(node)
{
	...
	http.open('POST', '/private/ajax/updatearticles.php', true);
...
<?php
if (isset($_GET['del']))
	echo 'swal("מחיקה", "המאמר נמחק בהצלחה.", "success");';
?>
</script>
</div>
</body>
