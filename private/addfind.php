...
if (isset($_POST['name'], $_POST['text'], $_FILES['image']))
{
	...
	if (empty($_POST['name']))
		$validation['nameempty'] = 'יש להזין שם/כותרת.';
	...
	if (empty($validation))
	{
		$result = $mysqli->query('SELECT MAX(imageOrder) imageOrder FROM lost_step');
		...
		else
		{
			...
			$stmt = $mysqli->prepare('INSERT INTO lost_step VALUES (?, ?, ?, ?)');
...
<div id="content">
<div id="contentInner">
<form action="/private/addfind.php" method="post" enctype="multipart/form-data">
<fieldset>
<h3>הוספת תוכן לאבד לי הכלב</h3>
<input type="text" name="name" placeholder="שם / כותרת" required maxlength="45" <?php if (!empty($validation)) echo 'value="' . $_POST['name'] . '"'; ?>>
<input type="file" name="image" accept="image/*" required title="בחר תמונה">
<textarea class="tinymce" name="text" placeholder="טקסט חופשי"><?php if (!empty($validation)) echo $_POST['text']; ?></textarea>
