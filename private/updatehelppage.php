...
if (isset($_GET['del']))
{
	...
	{
		$result = $mysqli->query('SELECT image FROM help WHERE position=' . $_GET['del']);
		$row = $result->fetch_assoc();
		$result->free();
		if ($row)
		{
			unlink($_SERVER['DOCUMENT_ROOT'] . '/images/pages/' . $row['image']);
			$mysqli->query('DELETE FROM help WHERE position=' . $_GET['del']);
		}
	}
}
?>
...
<div id="contentInner">
<h2>עדכון תוכן לאיך ניתן לעזור</h2>
<table class="sortable">
<thead><tr><th>מיקום</th><th>כותרת</th><th>תמונה</th><th>עריכה</th><th>מחיקה</th></tr></thead>
<tbody>
<?php
$result = $mysqli->query('SELECT position, title, image FROM help ORDER BY position');
...
<td data-label="עריכה"><a title="עריכה" href="/private/updatehelp.php?id=' . $row['position'] . '">עריכה</a></td>
...
function del(num)
{
	...
	(
		...
		function()
		{
			self.location.href = '/private/updatehelppage.php?del=' + num;
		}
	);
}
function edit(node)
...
function save(node)
{
	...
	http.open('POST', '/private/ajax/updatehelppage.php', true);
