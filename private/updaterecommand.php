...
$result = $mysqli->query('SELECT * FROM recommand WHERE imageOrder=' . $_GET['id']);
...
if (isset($_POST['order'], $_POST['name'], $_POST['text'], $_FILES['image']))
{
	...
	{
		$result = $mysqli->query('SELECT imageOrder FROM recommand WHERE imageOrder=' . $_POST['order']);
	...
	if (empty($_POST['name']))
		$validation['nameempty'] = 'יש להזין שם/כותררת.';
	elseif (mb_strlen($_POST['name']) > 45)
		$validation['namelong'] = 'השם/כותרת לא יכול להכיל יותר מ-45 תווים.';
	...
	if (empty($validation))
	{
		...
		{
			unlink($_SERVER['DOCUMENT_ROOT'] . '/images/recommands/' . $row2['image']);
			$image_name = mt_rand(1, time()) . time() . '.' . $file_type;
			$new_name = $_SERVER['DOCUMENT_ROOT'] . '/images/recommands/' . $image_name;
			move_uploaded_file($_FILES['image']['tmp_name'], $new_name);
		}
		$stmt = $mysqli->prepare('UPDATE recommand SET imageOrder=?, image=?, imageName=?, imageCaption=? WHERE imageOrder=?');
...
<div id="content">
<div id="contentInner">
<form action="/private/updaterecommand.php?id=<?php echo $_GET['id']; ?>" method="post" enctype="multipart/form-data">
<fieldset>
<h3>עדכון מומלצים</h3>
...
<input type="text" name="name" placeholder="שם / כותרת" required maxlength="45" value="<?php echo (empty($validation) ? $row2['imageName'] : $_POST['name']); ?>">
<?php
if (strpos($row2['image'], '.svg') !== false)
	$width = $height = 10000;
else
	list($width, $height) = getimagesize($_SERVER['DOCUMENT_ROOT'] . '/images/recommands/' . $row2['image']);
echo '<a href="/images/recommands/' . $row2['image'] . '" class="imageModal" title="הצג תמונה נוכחית" data-width="' . $width . '" data-height="' . $height . '">הצג תמונה נוכחית</a>';
?>
<input type="file" name="image" accept="image/*">
<textarea class="tinymce" name="text" placeholder="טקסט חופשי"><?php echo (empty($validation) ? $row2['imageCaption'] : $_POST['text']); ?></textarea>
<input type="submit" value="עדכן">
</fieldset>
</form>
<p><a title="חזרה לעדכון מומלצים" href="/private/updaterecommands.php">חזרה לעדכון מומלצים</a></p>
