...
if (isset($_POST['link'], $_FILES['image']))
{
	$validation = array();
	...
	if (empty($validation))
	{
		$result = $mysqli->query('SELECT MAX(imageOrder) imageOrder FROM board');
		...
		else
		{
			...
			$stmt->bind_param('iss', $row['imageOrder'], $image_name, $_POST['link']);
			$stmt->execute();
			$stmt->close();
		}
	}
