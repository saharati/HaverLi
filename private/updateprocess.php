...
if (isset($_POST['description'], $_POST['link'], $_FILES['image']))
{
	...
	if (empty($validation))
	{
		...
		if ($row)
			$stmt = $mysqli->prepare('UPDATE process SET description=?, image=?, imageLink=?');
		else
			$stmt = $mysqli->prepare('INSERT INTO process VALUES (?, ?, ?)');
...
<div id="content">
<div id="contentInner">
<form action="/private/updateprocess.php" method="post" enctype="multipart/form-data">
<fieldset>
<h3>עדכון תוכן תהליך אימוץ</h3>
<?php
if ($row)
{
