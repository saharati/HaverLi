...
$result = $mysqli->query('SELECT * FROM lost_step WHERE imageOrder=' . $_GET['id']);
...
if (isset($_POST['order'], $_POST['name'], $_POST['text'], $_FILES['image']))
{
	...
	{
		$result = $mysqli->query('SELECT imageOrder FROM lost_step WHERE imageOrder=' . $_POST['order']);
	...
	if (empty($validation))
	{
		...
		{
			unlink($_SERVER['DOCUMENT_ROOT'] . '/images/lost/' . $row2['image']);
			$image_name = mt_rand(1, time()) . time() . '.' . $file_type;
			$new_name = $_SERVER['DOCUMENT_ROOT'] . '/images/lost/' . $image_name;
			move_uploaded_file($_FILES['image']['tmp_name'], $new_name);
		}
		$stmt = $mysqli->prepare('UPDATE lost_step SET imageOrder=?, image=?, imageName=?, imageCaption=? WHERE imageOrder=?');
...
<div id="content">
<div id="contentInner">
<form action="/private/updatefind.php?id=<?php echo $_GET['id']; ?>" method="post" enctype="multipart/form-data">
<fieldset>
<h3>עדכון תוכן לאבד לי הכלב</h3>
...
if (strpos($row2['image'], '.svg') !== false)
	...
	list($width, $height) = getimagesize($_SERVER['DOCUMENT_ROOT'] . '/images/lost/' . $row2['image']);
echo '<a href="/images/lost/' . $row2['image'] . '" class="imageModal" title="הצג תמונה נוכחית" data-width="' . $width . '" data-height="' . $height . '">הצג תמונה נוכחית</a>';
?>
...
<p><a title="חזרה לעדכון תוכן" href="/private/updatefinds.php">חזרה לעדכון תוכן</a></p>
