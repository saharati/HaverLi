...
if (isset($_GET['del']))
{
	...
	{
		$result = $mysqli->query('SELECT image FROM volunteer WHERE imageOrder=' . $_GET['del']);
		$row = $result->fetch_assoc();
		$result->free();
		if ($row)
		{
			unlink($_SERVER['DOCUMENT_ROOT'] . '/images/volunteers/' . $row['image']);
			$mysqli->query('DELETE FROM volunteer WHERE imageOrder=' . $_GET['del']);
		}
	}
}
?>
...
<tbody>
<?php
$result = $mysqli->query('SELECT imageOrder, imageName, image FROM volunteer ORDER BY imageOrder');
while ($row = $result->fetch_assoc())
{
	if (strpos($row['image'], '.svg') !== false)
		$width = $height = 10000;
	else
		list($width, $height) = getimagesize($_SERVER['DOCUMENT_ROOT'] . '/images/volunteers/' . $row['image']);
	echo '<tr>
...
<td data-label="תמונה"><a class="imageModal" href="/images/volunteers/' . $row['image'] . '" data-width="' . $width . '" data-height="' . $height . '"><img src="/images/volunteers/' . $row['image'] . '"></a></td>
<td data-label="עריכה"><a title="עריכה" href="/private/updatevolunteer.php?id=' . $row['imageOrder'] . '">עריכה</a></td>
...
function del(num)
{
	...
	(
		{
			title: 'האם אתה בטוח?',
			text: 'האם אתה בטוח שברצונך למחוק מתנדב זה?',
			type: 'warning',
		...
		function()
		{
			self.location.href = '/private/updatevolunteers.php?del=' + num;
		}
	);
}
function edit(node)
...
function save(node)
{
	...
	http.open('POST', '/private/ajax/updatevolunteers.php', true);
...
<?php
if (isset($_GET['del']))
	echo 'swal("מחיקה", "המתנדב נמחק בהצלחה.", "success");';
?>
</script>
</div>
</body>
