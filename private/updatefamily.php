...
$result = $mysqli->query('SELECT * FROM family WHERE imageOrder=' . $_GET['id']);
...
if (isset($_POST['order'], $_POST['text'], $_FILES['image']))
{
	$validation = array();
	...
	{
		$result = $mysqli->query('SELECT imageOrder FROM family WHERE imageOrder=' . $_POST['order']);
	...
	if (empty($validation))
	{
		...
		{
			unlink($_SERVER['DOCUMENT_ROOT'] . '/images/families/' . $row2['image']);
		...
		$stmt = $mysqli->prepare('UPDATE family SET imageOrder=?, image=?, imageCaption=? WHERE imageOrder=?');
		$stmt->bind_param('issi', $_POST['order'], $image_name, $_POST['text'], $_GET['id']);
		$stmt->execute();
		$stmt->close();
		$_GET['id'] = $_POST['order'];
		$result = $mysqli->query('SELECT * FROM family WHERE imageOrder=' . $_GET['id']);
...
<div id="content">
<div id="contentInner">
<form action="/private/updatefamily.php?id=<?php echo $_GET['id']; ?>" method="post" enctype="multipart/form-data">
<fieldset>
<h3>עדכון משפחות מאושרות</h3>
...
if (strpos($row2['image'], '.svg') !== false)
	...
	list($width, $height) = getimagesize($_SERVER['DOCUMENT_ROOT'] . '/images/families/' . $row2['image']);
echo '<a href="/images/families/' . $row2['image'] . '" class="imageModal" title="הצג תמונה נוכחית" data-width="' . $width . '" data-height="' . $height . '">הצג תמונה נוכחית</a>';
?>
...
<input type="submit" value="עדכן משפחה">
</fieldset>
</form>
