...
if (isset($_GET['del']))
{
	...
	{
		$result = $mysqli->query('SELECT image FROM lost_step WHERE imageOrder=' . $_GET['del']);
		$row = $result->fetch_assoc();
		$result->free();
		if ($row)
		{
			unlink($_SERVER['DOCUMENT_ROOT'] . '/images/lost/' . $row['image']);
			$mysqli->query('DELETE FROM lost_step WHERE imageOrder=' . $_GET['del']);
		}
	}
}
?>
...
<div id="contentInner">
<h2>עדכון תוכן לאבד לי הכלב</h2>
<table class="sortable">
<thead><tr><th>מיקום</th><th>שם</th><th>תמונה</th><th>עריכה</th><th>מחיקה</th></tr></thead>
<tbody>
<?php
$result = $mysqli->query('SELECT imageOrder, imageName, image FROM lost_step ORDER BY imageOrder');
while ($row = $result->fetch_assoc())
{
	if (strpos($row['image'], '.svg') !== false)
		$width = $height = 10000;
	else
		list($width, $height) = getimagesize($_SERVER['DOCUMENT_ROOT'] . '/images/lost/' . $row['image']);
	echo '<tr>
...
<td data-label="תמונה"><a class="imageModal" href="/images/lost/' . $row['image'] . '" data-width="' . $width . '" data-height="' . $height . '"><img src="/images/lost/' . $row['image'] . '"></a></td>
<td data-label="עריכה"><a title="עריכה" href="/private/updatefind.php?id=' . $row['imageOrder'] . '">עריכה</a></td>
...
function del(num)
{
	...
	(
		...
		function()
		{
			self.location.href = '/private/updatefinds.php?del=' + num;
		}
	);
}
function edit(node)
...
function save(node)
{
	...
	http.open('POST', '/private/ajax/updatefinds.php', true);
