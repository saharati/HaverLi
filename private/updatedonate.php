...
if (isset($_POST['description'], $_POST['link'], $_FILES['image']))
{
	...
	if (empty($validation))
	{
		...
		if ($row)
			$stmt = $mysqli->prepare('UPDATE donate SET description=?, image=?, imageLink=?');
		else
			$stmt = $mysqli->prepare('INSERT INTO donate VALUES (?, ?, ?)');
...
<div id="content">
<div id="contentInner">
<form action="/private/updatedonate.php" method="post" enctype="multipart/form-data">
<fieldset>
<h3>עדכון תוכן תרומות</h3>
<?php
if ($row)
{
