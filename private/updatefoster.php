...
$result = $mysqli->query('SELECT * FROM foster');
...
if (isset($_POST['description'], $_POST['link'], $_FILES['image']))
{
	...
	if (empty($validation))
	{
		...
		if ($row)
			$stmt = $mysqli->prepare('UPDATE foster SET description=?, image=?, imageLink=?');
		else
			$stmt = $mysqli->prepare('INSERT INTO foster VALUES (?, ?, ?)');
...
<div id="content">
<div id="contentInner">
<form action="/private/updatefoster.php" method="post" enctype="multipart/form-data">
<fieldset>
<h3>עדכון תוכן בתי אומנה</h3>
<?php
if ($row)
{
