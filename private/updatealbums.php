...
<thead><tr><th>עדכון אחרון</th><th>שם</th><th>חשיבות</th><th>פעולות</th></tr></thead>
<tbody>
<?php
...
$result = $mysqli->query('SELECT id, DATE_FORMAT(postDate, "%d/%m/%Y") ps, name, important FROM album WHERE ' . $sql . '1=1 ORDER BY postDate DESC LIMIT ' . $previousRows . ', ' . $rowsPerPage);
...
$result->free();
pagination($_GET['page'], ceil($row['c'] / $rowsPerPage), '/private/updatealbums.php?text=' . $_GET['text'] . '&page=');
?>
