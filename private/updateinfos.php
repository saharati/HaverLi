...
if (isset($_GET['del']))
{
	...
	{
		$result = $mysqli->query('SELECT image FROM info WHERE position=' . $_GET['del']);
		$row = $result->fetch_assoc();
		$result->free();
		if ($row)
		{
			unlink($_SERVER['DOCUMENT_ROOT'] . '/images/pages/' . $row['image']);
			$mysqli->query('DELETE FROM info WHERE position=' . $_GET['del']);
		}
	}
}
?>
...
<div id="contentInner">
<h2>עדכון תוכן למידע למאמץ</h2>
<table class="sortable">
<thead><tr><th>מיקום</th><th>כותרת</th><th>תמונה</th><th>עריכה</th><th>מחיקה</th></tr></thead>
<tbody>
<?php
$result = $mysqli->query('SELECT position, title, image FROM info ORDER BY position');
...
function del(num)
{
	...
	(
		...
		function()
		{
			self.location.href = '/private/updateinfos.php?del=' + num;
		}
	);
}
function edit(node)
...
function save(node)
{
	...
	http.open('POST', '/private/ajax/updateinfos.php', true);
