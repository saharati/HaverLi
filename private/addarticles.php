...
if (isset($_POST['name'], $_POST['text'], $_FILES['image']))
{
	...
	if (empty($_POST['name']))
		$validation['nameempty'] = 'יש להזין כותרת למאמר.';
	...
	if (empty($validation))
	{
		$result = $mysqli->query('SELECT MAX(imageOrder) imageOrder FROM article');
		...
		else
		{
			...
			$stmt = $mysqli->prepare('INSERT INTO article (imageOrder, image, imageName, imageCaption) VALUES (?, ?, ?, ?)');
...
<div id="content">
<div id="contentInner">
<form action="/private/addarticles.php" method="post" enctype="multipart/form-data">
<fieldset>
<h3>הוספת מאמרים</h3>
<input type="text" name="name" placeholder="כותרת המאמר" required maxlength="45" <?php if (!empty($validation)) echo 'value="' . $_POST['name'] . '"'; ?>>
<input type="file" name="image" accept="image/*" required title="בחר תמונה">
<textarea class="tinymce" name="text" placeholder="תוכן המאמר"><?php if (!empty($validation)) echo $_POST['text']; ?></textarea>
<input type="submit" value="הוסף מאמר">
</fieldset>
</form>
...
if (isset($validation))
{
	echo '<script>';
	if (empty($validation))
		echo 'swal("הוספת מאמר", "המאמר נוסף בהצלחה.", "success");';
	else
	{
