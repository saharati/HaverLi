...
if (isset($_POST['imageOrder'], $_POST['newImageOrder']) && is_numeric($_POST['imageOrder']) && $_POST['imageOrder'] > 0 && $_POST['imageOrder'] < 100)
{
	...
	{
		$result = $mysqli->query('SELECT imageOrder FROM recommand WHERE imageOrder=' . $_POST['newImageOrder']);
		$row = $result->fetch_assoc();
		$result->free();
		if ($row)
			echo 'מספר זה כבר קיים.';
		else
			$mysqli->query('UPDATE recommand SET imageOrder=' . $_POST['newImageOrder'] . ' WHERE imageOrder=' . $_POST['imageOrder']);
	}
}
?>