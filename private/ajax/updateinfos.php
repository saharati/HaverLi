...
if (isset($_POST['imageOrder'], $_POST['newImageOrder']) && is_numeric($_POST['imageOrder']) && $_POST['imageOrder'] > 0 && $_POST['imageOrder'] < 100)
{
	...
	{
		$result = $mysqli->query('SELECT position FROM info WHERE position=' . $_POST['newImageOrder']);
		$row = $result->fetch_assoc();
		$result->free();
		if ($row)
			echo 'מספר זה כבר קיים.';
		else
			$mysqli->query('UPDATE info SET position=' . $_POST['newImageOrder'] . ' WHERE position=' . $_POST['imageOrder']);
	}
}
?>